...
namespace EnvCrypt.Core.Verb.DecryptEntry
{
        ...
        where TKey : KeyBase
    {
        ...
        public DecryptEntryWorkflow(IKeyLoader<TKey, KeyFromFileDetails> keyLoader, IDatLoader datLoader, EntriesDecrypter<TKey> entriesDecrypter)
        {
        ...
        public IList<EntriesDecrypterResult> Run(DecryptEntryWorkflowOptions options)
        {
            ...
            for (uint keyPathI = 0; keyPathI < options.KeyFilePaths.Count; keyPathI++)
            {
                var loadedKey = _keyLoader.Load(
                    new KeyFromFileDetails(){ FilePath = options.KeyFilePaths[(int)keyPathI] });
                keys.Add(loadedKey);
            }
